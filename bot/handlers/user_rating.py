...
def register_rating_handlers(application):
    ...
    excluded_commands = [
        ...
        '📍 Мой заказ', '📋 Мои заказы', 'ℹ️ Помощь', 'ℹ️ О сервисе', '📞 Связаться',
    ...
    comment_conv = ConversationHandler(
        ...
        fallbacks=[
            ...
            MessageHandler(filters.Regex('^(📍 Мой заказ|📋 Мои заказы|ℹ️ Помощь|ℹ️ О сервисе|📞 Связаться|📜 Правила пользования|🛣 Межгород|🔙 В главное меню|🚖 Заказать такси)$'), 
